...
$(document).ready(function() {
    ...
    // 绑定提交分数事件 回车 点击都可以提交
    $("#submit_score_button").bind("submit", submitScore);
    $("#submit_score_button").bind("click", submitScore);

    newgame();
});

function setBodyWrapHeight() {
    var body_wrap_height = $(window).height() - 70;
    body_wrap_height = body_wrap_height < 650 ? 650 : body_wrap_height;
    $("#body_wrap").height(body_wrap_height);
}
 ...
 * 填充分数排行榜
 */
function completeRankingBox() {
    $.post("/responseFile/2048_page_response.jsp?game_2048_page_request=get_ranking", function callback(data, status) {
        if(status == "success") {
            var game_2048_JSONObject = $.parseJSON(data);
            $("#rank_1").html(game_2048_JSONObject.ranking[0]);
            $("#rank_2").html(game_2048_JSONObject.ranking[1]);
            $("#rank_3").html(game_2048_JSONObject.ranking[2]);
            $("#hit_legend").html("您的分数是 : " + score + "  请留下您的尊姓大名");
        }
    });
}
...
function submitScore() {
    ...
    if(player_name.length < 1 || player_name > 15) {
        return;
    }
    $.post("/responseFile/2048_page_response.jsp?game_2048_page_request=submit_ranking&player_name=" + player_name + "&score=" + score);
    $("#ranking_close").click();
}
